...
def get_aggregation_params(recipe_config):
    ...
    if _p('input_features_definition') == 'auto': #TODO FIXME don't start computing yet!!
        ...
        ratio_threshold = _p('ratio_threshold', 0.1)
    ...
    if _p('use_temporal_window'):
        ...
        params.num_windows_per_type = _p('num_windows_per_type', 1)
...
def get_transform_params(recipe_config):
    ...
    if recipe_config.get('use_buffer'):
        transform_params.buffer_unit = recipe_config.get('buffer_unit', None)
        ...
        try:
            ...
            raise ValueError('buffer_width must be an integer')
...
def _infer_column_types(dataset, ignored_columns=[], row_limit=1000000, threshold=1):
        ...
        if(df[col].dtype == np.float64 or df[col].dtype == np.int64):
            ...
            if num_unique > 1:
                ...
                if value_diversity_percentage >= threshold:
                    logger.info(" -> Column '{}' is numerical (value_diversity_percentage: {})".format(col, value_diversity_percentage))
                    numerical_columns.append(col)
                else:
                    logger.info(" -> Column '{}' is categorical (value_diversity_percentage: {})".format(col, value_diversity_percentage))
