...
class FeatureAggregator:
    ...
    def transform(self, input_dataset, output_dataset, transform_params):
        ...
        full_query_sql = sql_generation.make_full_transform_query(queries, input_dataset, self.params, resolved_transform_params)
        SQLExecutor2.exec_recipe_fragment(output_dataset, query=full_query_sql)
    ...
    def _compute_most_recent_timestamp(self, timestamp_column, dataset):
        ...
        df = SQLExecutor2(dataset=dataset).query_to_df(sql)
        if df.empty:
    ...
    def _compute_distinct_values(self, column, dataset):
        ...
        res = SQLExecutor2(dataset=dataset).query_to_iter(sql_query)
    ...
    def _check_populations(self):
        ...
        for population_combination in populations_values:
            ...
            for combination_val in population_combination:
                ...
                single_cond_expr = Column(condition_column).in_(combination_val[1])
...
class WindowWidthUnit:
    ...
    def get_available_units(time_reference_type):
        if time_reference_type == TimeReferenceType.FIXED_TIMESTAMP:
            return [WindowWidthUnit.DAY, WindowWidthUnit.MONTH, WindowWidthUnit.YEAR]
    ...
    def get_all_available():
        return [WindowWidthUnit.ROW, WindowWidthUnit.DAY, WindowWidthUnit.MONTH, WindowWidthUnit.YEAR]
...
class AggregationParams:
    ...
    def __init__(self,
            ...
            selected_aggregation_types=[AggregationType.FREQUENCY, AggregationType.RECENCY],
